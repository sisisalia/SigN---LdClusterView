...
function drawManhattanPlot(table, eqtls, newRow, plotId, leftPlotId, rightPlotId, selected_study) {
    ...
    //     return d.neglog10p_unit;
    // }));
    // var scale_end = Math.max.apply(null, $.map(eqtls, function(d, i){
    //     return d.neglog10p_unit;
    // }));
    // var scale = d3.scale.linear().range([0, height-31]).domain([scale_start, scale_end]);
    ...
    var axis = d3.svg.axis().scale(d3.scale.linear().range([0, height-31]).domain([start, end]).nice()).tickFormat(d3.format(".1e")).orient("right");
