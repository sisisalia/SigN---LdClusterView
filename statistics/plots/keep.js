function MST(data,spearman, kruskal, chi){
    ...
    function createMST(choosen_links)
    {
        ...
        $("input[name='nodes']").change(function () {
            ...
            if (highlighted.length != 0) {
                ...
                mst_path = [];
                highlightPath(choosen_links,highlighted);
                console.log(mst_path);
                toggle = 1;
            } else {
                returnOpacity();
            }
    ...
    function highlightPath(links, highlighted_nodes) {
        // get all the nodes
        var nodes = [];
        for (var i = 0; i < links.length; i++) {
            var index = nodes.indexOf(links[i].source.name);
            if (index == -1) nodes.push(links[i].source.name);
            var index = nodes.indexOf(links[i].target.name);
            if (index == -1) nodes.push(links[i].target.name);
        }
        var obj = {};
        // get all the neighbours of each nodes
        for (var i = 0; i < nodes.length; i++) {
            obj[nodes[i]] = getNeighbour(links, nodes[i]);
        }
        for(var i = 0; i < highlighted_nodes.length; i++){
            if(!highlighted_nodes[i+1]) continue;
            var start_node = highlighted_nodes[i];
            var end_node = highlighted_nodes[i+1];
            mst_path.push(start_node);
            var result = find_path(obj,start_node,end_node,null);
            if(result == 1) {
                return;
            }
        }
    ...
    function find_path(obj,start_node,end_node,prev_node){
        var start_node_neighbour = obj[start_node].slice(0);
        var index = start_node_neighbour.indexOf(prev_node);
        start_node_neighbour.splice(index,1);
        // see if the node are next to each other
        var index = start_node_neighbour.indexOf(end_node);
        var end_node_neighbour = obj[end_node];
        for(var i = 0; i < start_node_neighbour.length; i++){
            if(start_node_neighbour[i] == end_node){
                mst_path.push(end_node);
                return 1;
            }
            if(end_node_neighbour.indexOf(start_node_neighbour[i]) != -1) {
                mst_path.push(start_node_neighbour[i]);
                mst_path.push(end_node);
                return 1;
            }
            var next_node = start_node_neighbour[i];
            mst_path.push(next_node);
            var result = find_path(obj,next_node,end_node,start_node);
            if(result == 1){
                return 1;
            }
        }
    ...
    function getNeighbour(links,node){
        var targets = [];
        for(var i = 0; i < links.length; i++){
            if(links[i].source.name == node){
                targets.push(links[i].target.name);
                continue;
            }
            if(links[i].target.name == node){
                targets.push(links[i].source.name);
                continue;
            }
        }
        return targets;
